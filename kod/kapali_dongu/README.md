...
| `cl_selfcheck.py` | `verify(P)` kapıları G1–G9 ve CMA-ES için `fitness(P)`. G9 yapısal testtir: geri beslemeyi faz-ortalaması sabitle (`meanff`) değiştirir. |
| `cl_optimize.py` | CMA-ES (paralel); `veri/kapali_dongu/cl_best.json`'ı ılık başlangıç olarak okur ve oraya yazar. |
| `cl_teslim_9of9.py` | 9/9 kapıyı geçen parametre setiyle teslim koşusu. Çıktı: `sekiller/cl_teslim_9of9.png` + `veri/kapali_dongu/cl_teslim_9of9.npz`. |
...
- **`cl_emergent.py` ızgarayı import anında yükler.** `veri/kapali_dongu/cl_grid3d.npz` yerinde
  ...
  (`stage0_grid.py`) hiçbir zaman depoya girmedi — **yeniden üretilemez birincil varlıktır.**
